<template>
  ...
  <script>
import { suppliesDrawDetaillists,suppliesDrawDetailDelete} from "@/api/index";
import tables from "@/components/table";
import { formatDay } from "@/utils/date";
import newstaffak from "./NewStaffak/index";
import EditorialStaffak from "./EditorialStaffak/index";
  ...
  methods: {
    ...
    DeletePeople(val) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          suppliesDrawDetailDelete({ id: val.id }).then((res) => {
            console.log(res);
            if (res.code == 200) {
              this.toUpdate();
              this.$message({
    ...
    inventorylist(val) {
      suppliesDrawDetaillists(val).then((res) => {
        // console.log(res)
