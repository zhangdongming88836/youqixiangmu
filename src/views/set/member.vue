<template>
  <div id="PersonnelInformation">
    ...
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item class="item">
        ...
        <el-button v-if="NewPermissionsOfUsermanagement" type="primary" @click="addStaff">新增</el-button>
      </el-form-item>
    </el-form>
    <div class="borfer">
      <tables
      ...
      >
        <template v-slot:status="data">
          <el-button
          v-if="UserManagementeditPermission"
            type="text"
            size="small"
            @click="EditorialStaffnm(data.data)"
            >编辑</el-button
          >
              <el-button v-if="UserManagementStatusPermissions" type="text" size="small" @click="DeletePeople(data.data)"
            >{{data.data.status == '未启用'?'可用':'停用'}}</el-button
          >
        </template>
      </tables>
    </div>
  ...
  <script>
import { userlists, userDelete } from "@/api/index";
import tables from "@/components/table";
import { formatDay } from "@/utils/date";
import newstaffah from "./NewStaffah/index";
import EditorialStaffah from "./EditorialStaffah/index";
...
export default {
  ...
  data() {
    return {
      //新增按钮权限
      NewPermissionsOfUsermanagement:false,
      //编辑按钮权限
      UserManagementeditPermission:false,
      //状态按钮权限
      UserManagementStatusPermissions:false,
      name: "",
    //   status:'',
    //   statuss:[
    //     {
  ...
  created() {
    ...
    this.inventorylist(obj);
     this.$store.state.user.buttonPermission.forEach(item=>{
       item.authorityCode == 'us-002'? this.NewPermissionsOfUsermanagement = true : false
       item.authorityCode == 'us-003'? this.UserManagementeditPermission = true : false
       item.authorityCode == 'us-004'? this.UserManagementStatusPermissions = true : false
     })
  },
};
</script>
