    ...
    >
      <div class="contebn">
        <el-form
          ...
          class="demo-ruleForm"
        >
          <el-form-item label="维修计划名称" prop="planId">
            <el-select
              v-model="ruleForm.planId"
              placeholder="请选择维修计划名称"
            >
              <el-option
          ...
          <el-form-item label="计划维修日期" prop="time">
            <div class="block">
              <el-date-picker
          ...
          <el-form-item label="油漆套餐" prop="paintMealId">
            <el-select
              v-model="ruleForm.paintMealId"
              placeholder="请选择"
              @change="PaintPackagechanges"
            >
              <el-option
          ...
          <div>
            <label style="margin-left: 6px">工单涂层明细表</label>
            <template>
              <div style="margin-left: 99px">
                <table id="customers">
                  ...
                  <template
                    v-for="(item, index) in ruleForm.workOrderPaintMealList"
                  >
                    <tr v-if="item.status == 0" :key="index">
                      <td>{{ index + 1 }}</td>
                      <!-- <td>
          ...
          <!--套餐除锈 -->
          <div>
            <label style="margin-left: 6px">工单除锈信息</label>
            <template>
              <div style="margin-left: 99px">
                <table id="customersa">
                  <tr>
                  ...
                  <template
                    v-for="(item, index) in ruleForm.workOrderDescMealList"
                  >
                    <tr v-if="item.status == 0" :key="index">
                      <td>{{ index + 1 }}</td>
                      <!-- <td>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="toExamine(ruleForm.id)" type="success">审核</el-button>
...
import {
  workOrderInsert,
  partListToSelect,
  formulaSetMealListToSelect,
  employeeInsertTwo,
  formulaSetMealToUpdate,
  sysOrganizationListFour,
  workOrderPaintMealToUpdate,
  workOrderDescMealToUpdate,
  maintenancePlanninglistTwo,
  workOrderToUpdate,
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ruleForm: {
        ...
        remark: "",
        id:"",
        workOrderPaintMealList: [],
        workOrderDescMealList: [],
      },
      rules: {
        planName: [
          {
  ...
  methods: {
    //审核
    toExamine(val){
    workOrderUpdateState({id:val}).then(res=>{
     //console.log(res)
       if (res.code == 200) {
              this.$message({
                message: `${res.msg}`,
    ...
    PaintPackagechanges(val){
       ...
       formulaSetMealToUpdate(val).then(res=>{
          console.log(res,'************')
         res.data.formulaPaintMealList.forEach(item =>{
           var obj = {};
            obj.materialsName = item.materialsName
            obj.paintMaterialId = item.paintMaterialId
            obj.coat = item.coat
            ...
            this.ruleForm.workOrderPaintMealList.push(obj)
         }) 
         //console.log(this.ruleForm.formulaPaintMealList);
         res.data.formulaDescaMealList.forEach(item=>{
           var obj = {};
           obj.descaling = item.descaling;
           obj.status = item.status;
           this.ruleForm.workOrderDescMealList.push(obj)
         })
    ...
    open() {
      ...
      maintenancePlanninglistTwo({}).then(res=>{
        //console.log(res,'dsdsa');
        this.planIds = res.data;
      })
      //获取详情信息
      workOrderToUpdate(this.ruleForm.id).then((res) => {
        console.log(res, 123456);
        this.ruleForm.paintMealId = res.data.paintMealId;
        ...
        this.ruleForm.workOrderDescMealList =
          res.data.workOrderDescMealList;
        this.ruleForm.workOrderPaintMealList =
          res.data.workOrderPaintMealList;
      });
    },
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          var arr = [];
          var arrws = [];
          this.ruleForm.workOrderPaintMealList.forEach((item) => {
            var obj = {};
            obj.materialsName = item.materialsName;
            obj.coat = item.coat;
            obj.coatingThickness = item.coatingThickness;
          ...
          this.ruleForm.workOrderPaintMealList = arr;
         // console.log(this.ruleForm.maintenancePlanningPaintMealList, 987);
          this.ruleForm.workOrderDescMealList.forEach((item) => {
            var obj = {};
            obj.descaling = item.descaling;
            obj.status = item.status;
            obj.id = item.id;
          ...
          this.ruleForm.workOrderDescMealList = arrws;
          //console.log(this.ruleForm.maintenancePlanningDescaMealList, 654);
          workOrderUpdate(this.ruleForm).then((res) => {
            if (res.code == 200) {
              this.$message({
                message: `${res.msg}`,
                type: "success",
